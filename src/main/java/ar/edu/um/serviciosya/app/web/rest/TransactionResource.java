...
import ar.edu.um.serviciosya.app.service.TransactionService;
...
/**
 * REST controller for managing {@link ar.edu.um.serviciosya.app.domain.Transaction}.
 */
@RestController
@RequestMapping("/api")
public class TransactionResource {
    ...
    public ResponseEntity<List<TransactionDTO>> getAllTransactions(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("turn-is-null".equals(filter)) {
            log.debug("REST request to get all Transactions where turn is null");
            return new ResponseEntity<>(transactionService.findAllWhereTurnIsNull(),
                    HttpStatus.OK);
        }
